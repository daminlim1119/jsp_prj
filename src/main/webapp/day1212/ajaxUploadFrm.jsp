...
if(!flag){
	alert("업로드 가능한 파일은 gif,jpg,png,bmp만");
	return;
}//end if
...
$("#upfile").change(function(evt){ 
	//미리보기
	//1.HTML File Control접근
	var file=evt.target.files[0];
	//2.파일을 읽기 위한 스트림 생성
	var reader=new FileReader();
	//3.File 객체의 onload 이벤트 핸들러를 설정하여
	//<img 태그에 src속성에 파일명 설정
  ...
  $( function() {
    ...
    $("#btnFileUpload").click(function(){
    	...
    	//ajax로 파일 업로드
    	alert($("#frm")[0]);//파라메터 전송방식..
    	var formData=new FormData(form);//파라메터 전송방식->binary파일전송 방식
    	
    	$.ajax({
    		
    		url:"ajaxFileProcess.jsp",
    		contentType:false,/*ajax의 요청방식을 binary전송 방식으로 변경*/
    		processData:false,/*ajax요청에 쿼리스트링 막기*/
    		type:"post",
    		data:formData,/*binary전송 방식으로 설정된 폼*/
    		dataType:"JSON",
    		error:function(xhr){
    			alert("죄송 파일 전송에 문제가..");
    			
    		},
    		succes:function(jsonObj){
    			if(jsonObj.resultFlag){//true
...
<body>
  ...
  ================================================== -->
	...
	<!-- enctype 파라메터 전송방식 -->		
	<!-- binary 전송방식  -->		
	<form action="uploadFrmProcess.jsp" id="frm" method="post" enctype="multipart/form-data">
		<label>업로더</label>
		...
		<input type="button" value="ajax파일업로드" id="btnFileUpload" class="btn btn-info btn-sm" /><br>
