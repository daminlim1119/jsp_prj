...
pDTO.setEmail(request.getParameter("mail")+"@"+request.getParameter("domain"));
				...
				ms.joinMember(pDTO);//값을 받을 DTO를 서비스 객체에 전달할 수 있다

WebMemberService wms=WebMemberService.getInstance();
				
//한정적인 자원을 동시에 사용할 때 조회를 먼저 수행한 후 다음 작업으로 진행
boolean idFlag=wms.searchId(pDTO.getId());
if(!idFlag){//아이디를 선점한 경우 
	%>
입력하신 ${param.id }는 이미 사용 중입니다.<br>
다른 아이디를 입력해보세요<a href="javascript:history.back()" class="btn btn-success">뒤로</a>	
	<% 
}else{
	String key="a123456789012345";
boolean addFlag=wms.addMember(pDTO,key);
...
(<jsp:getProperty property="id" name="pDTO"/>)님
회원가입을 축하드립니다.<br>
...
<jsp:getProperty property="language" name="pDTO"/>
<%//jsp:getProperty를 사용하면 배열의 요소를 출력할 수 없다
String[]lang=request.getParameterValues("language");
if(lang !=null){//nullpointerexception발생 막기위해
...
<c:forEach var="lang" items="${ paramValues }">
<c:out value="${lang }"/></c:forEach>
<a href="${CommonURL }/index.html">첫 화면</a>
</div>
